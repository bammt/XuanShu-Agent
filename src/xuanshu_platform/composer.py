...
返回 ArchitectureStageDecision JSON，必须包含：summary（面向用户的应用简介，1-2 句）、kind、process、interaction_mode、agents、tasks。
   ...
   multi_turn 的首节点必须是普通 Agent，且必须由 user_interaction=true 的普通 Agent 负责（首节点必须是普通 Agent，不是把 Agent 记录成输入状态的伪任务）；层级 Crew 仅 manager 可交互，其他成员把缺失信息汇报给 manager；single_run 禁止 ask_user。尊重已确认的 kind、interaction_mode、inputs 和资源，不返回 generation 专属配置。''',
...
交互约束：multi_turn 的第一个执行节点必须绑定 user_interaction=true 的普通 Agent。层级 Crew 中只有 manager/允许委派的管理 Agent 可以 user_interaction=true；其他 Agent 必须为 false，缺少信息时在任务结果中明确汇报给 manager，由 manager 调用 ask_user。
